...
import { IXcelerateExecutor } from "./interfaces/IXcelerateExecutor.sol";
import { IXcelerateTreasury } from "./interfaces/IXcelerateTreasury.sol";
...
abstract contract SendLibBase is MessageLibBase, Ownable {
    ...
    function _payExecutor(
        ...
        executorFee = IXcelerateExecutor(_executor).assignJob(_dstEid, _sender, _msgSize, _executorOptions);
        if (executorFee > 0) {
    ...
    function _quote(
        ...
        nativeFee += IXcelerateExecutor(config.executor).getFee(_dstEid, _sender, _msgSize, executorOptions);
